...
\title{Invariância a Translação na Super-Resolução por Tiles:
...
\begin{IEEEkeywords}
super-resolução, invariância a translação, equivariância, ESRGAN, junção de tiles,
...
\textbf{o gerador é invariante a translação?} Se fosse, dois tiles que cobrem a mesma
...
Camadas convolucionais são, por construção, equivariantes a translação; CNNs completas,
porém, não são estritamente invariantes, por causa de \textit{padding} e
subamostragem~\cite{b_shiftinv}. Este trabalho é uma verificação empírica direta dessa
ressalva em um gerador de SR de produção. Nossas contribuições são:
\begin{itemize}
  ...
  distância à borda do tile --- que localiza a não-invariância e a atribui ao campo
...
% ===========================================================================
\subsection{Super-resolução por tiles e invariância a translação}
...
\subsection{Fontes de não-invariância no Satlas}
...
\subsection{Protocolo de medida da invariância}
...
Para localizar a não-invariância, acumula-se a diferença absoluta média por coluna de
...
saídas \emph{divergem}, e a divergência cresce de forma monotônica à medida que a
...
\textbf{O gerador não é invariante a translação.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_consistency_pooled.png}}
...
monotônica ao reduzir a sobreposição revela a não-invariância; o salto a $100\%$
corresponde ao controle $\Delta=0$.}
\label{fig:consistency}
\end{figure}

\subsection{Onde a não-invariância se concentra}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_border_pooled.png}}
...
$\sim$$2{,}0$\,DN no interior, localizando a não-invariância junto ao
\textit{zero-padding}.}
\label{fig:border}
\end{figure}
...
erro de borda diagnosticado acima. Qualquer ponderação já reduz drasticamente a costura,
...
SSIM $0{,}959$). Notavelmente, a máscara \texttt{data-driven} crua ($1{,}09$ /
$32{,}4$\,dB) \emph{não} superou a gaussiana: embora principiada, o perfil empírico
carrega ruído de amostragem, ao passo que o decaimento suave do peso gaussiano casa bem
com a forma do perfil de erro da Fig.~\ref{fig:border}. É um resultado honesto que sugere
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_blend_pooled.png}}
...
\textbf{não-invariância é real e mensurável}: a auto-consistência degrada
...
deslocamentos inteiros explora a \emph{própria} não-invariância --- cada pixel é coberto
...
referência nítida). Em suma, o gaussiano é superado: \emph{Poisson com Dirichlet} domina
...
\label{sec:conclusao}
% ===========================================================================
Medimos, em modo \emph{zero-shot} e sobre Sentinel-2, o grau de (não-)invariância a
...
de forma \emph{pooled}. O gerador \textbf{não} é invariante: a discordância entre
...
a não-invariância na origem. \textbf{(4) Generalização} a outras tiles/biomas, sensores,
...
survey,'' \textit{IEEE Trans. Pattern Anal. Mach. Intell.}, vol. 43, no. 10, 2021.
